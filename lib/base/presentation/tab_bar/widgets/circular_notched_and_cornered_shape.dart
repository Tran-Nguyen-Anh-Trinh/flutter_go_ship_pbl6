  ...
  Path getOuterPath(Rect host, Rect? guest) {
    if (guest == null || !host.overlaps(guest)) {
      if (this.rightCornerRadius > 0 || this.leftCornerRadius > 0) {
        final double leftCornerRadius = this.leftCornerRadius * (animation?.value ?? 1);
        final double rightCornerRadius = this.rightCornerRadius * (animation?.value ?? 1);
        return Path()
    ...
    final notchRadius = guest.width / 2 * (animation?.value ?? 1);
    final leftCornerRadius = this.leftCornerRadius * (animation?.value ?? 1);
    final rightCornerRadius = this.rightCornerRadius * (animation?.value ?? 1);
    ...
    final p = List.filled(6, Offset.zero, growable: true);
